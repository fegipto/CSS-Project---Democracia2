...
public class DelegateSelectionService {
  ...
  /**
   * Contructs a new DelegateSelectionService object using a delegate repository, topic repository,
   * citizen repository
   *
   ...
   */
  @Autowired
  public List<CitizenDTO> getDelegates() {
    return delegateRepository.findAll().stream()
        .map(delegate -> new CitizenDTO(delegate))
