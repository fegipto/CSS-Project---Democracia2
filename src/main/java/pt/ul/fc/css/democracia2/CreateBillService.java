...
public class CreateBillService {
    ...
    @Autowired
    public List<TopicDTO> getTopics() { return topicRepository.getTopics().stream().
            map(TopicDTO::new).collect(Collectors.toList()); }

    public void chooseTopic(String topic) { this.topic = topicRepository.findByName(topic); }

    public void appendFile (byte[] file) { this.file = file; }

    public void submit() { billRepository.save(delegate.proposeBill(title, desc, file, validity, topic)); }
}
